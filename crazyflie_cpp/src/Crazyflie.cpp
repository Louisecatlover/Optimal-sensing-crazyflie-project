  ...
  , m_logger(logger)
{
  ...
  success = std::sscanf(link_uri.c_str(), "radio://%d/%d/%d/%d%c/%" SCNx64,
     &ack_enable_in, &m_devId, &channel, &datarate,
     &datarateType, &m_address) == 6;
  if (!success) {
    success = std::sscanf(link_uri.c_str(), "radio://%d/%d/%d/%d%c",
       &ack_enable_in, &m_devId, &channel, &datarate,
       &datarateType) == 5;
  ...
  if (success)
  {
...

    {
      ...
      if (!g_crazyradios[m_devId]) {
        ...
        g_crazyradios[m_devId]->setAckEnable(m_ackEnable);
  ...
  float yaw)
{
  ...
  if(m_ackEnable) sendPacket(request);
  else sendPacketDirect((uint8_t*)&request,sizeof(request));
}
  ...
  float z)
{
  ...
  if(m_ackEnable) sendPacket(position);
  else sendPacketDirect((uint8_t*)&position,sizeof(position));
}
  ...
  float qx, float qy, float qz, float qw)
{
  ...
  if(m_ackEnable) sendPacket(pose);
  else sendPacketDirect((uint8_t*)&pose,sizeof(pose));
}
...
void Crazyflie::sendPacketDirect(
  const uint8_t* data,
  uint32_t length)
{
  std::unique_lock<std::mutex> mlock(g_radioMutex[m_devId]);
  ...
  if (success)
  {
...

    {
      ...
      if (!g_crazyradios[m_devId]) {
        g_crazyradios[m_devId] = new Crazyradio(m_devId);
        g_crazyradios[m_devId]->enableLogging(LOG_COMMUNICATION);
         g_crazyradios[m_devId]->setAckEnable(ack_enable);
        //g_crazyradios[m_devId]->setAckEnable(true);
        g_crazyradios[m_devId]->setArc(0);
      }
    }
