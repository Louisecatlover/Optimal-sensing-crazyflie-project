...
#Subscribe msg
Pc,Pr,Pb,theta_c = None,None,None,None #UAVs ground truth
Pq,Vq = None,None #Target ground truth
Pq_hat,Vq_hat = None,None #Estimated target state
u_desired = None #Desired control inout
u1_bar, v1_bar = None,None #Target image point
#Publish msg
FOV = output_FOV()
...
def CBF():
	global A,b,tracking_distance,inter_uavs_distance,FOV
	#estimated state
	...
	A = np.array([np.append(np.append(-(np.dot(nc,r_qc_hat)*r_qc_hat/r_qc_norm_hat**3-nc/r_qc_norm_hat)/sqrt(1 - np.dot(nc,r_qc_hat)**2/r_qc_norm_hat**2),[0]*6),-np.dot(nc_dot,r_qc_hat)/r_qc_norm_hat/sqrt(1 - np.dot(nc,r_qc_hat)**2/r_qc_norm_hat**2)), \
	...
	b = np.array([gamma_fov*(theta_fov_fix - theta_qc_hat)-np.dot((np.dot(nc,r_qc_hat)*r_qc_hat/r_qc_norm_hat**3-nc/r_qc_norm_hat)/sqrt(1 - np.dot(nc,r_qc_hat)**2/r_qc_norm_hat**2),Vq_hat),\
				gamma_safe_car*(r_qc_norm_hat**2 - d_safe_car**2)-2*np.dot(-r_qc_hat,Vq_hat), \
