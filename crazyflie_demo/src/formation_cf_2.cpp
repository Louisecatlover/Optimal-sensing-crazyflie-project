...
void follow(vir& vir, geometry_msgs::PoseStamped& host_mocap, crazyflie_driver::VelocityWorld* vs, displacement &dis_host, geometry_msgs::PoseStamped& nbr_mocap, displacement &dis_nbr)
{
...
int main(int argc, char **argv)
{
	...
	while (ros::ok()) {
		...
		follow(vir1,host_mocap,&vs,dis1,host_mocap2,dis2);        //be careful of the sign
		vs.header.seq += 1;
        vs.header.stamp = ros::Time::now();
		local_vel_pub.publish(vs);
		follow(vir1,host_mocap2,&vs2,dis2,host_mocap,dis1);     //be careful of the sign
		vs2.header.seq += 1;
        vs2.header.stamp = ros::Time::now();
		local_vel_pub2.publish(vs2);
