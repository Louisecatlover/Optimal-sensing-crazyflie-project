...
geometry_msgs::PoseStamped host_mocap, host_mocap_target;
...
void follow(vir& vir, geometry_msgs::PoseStamped& host_mocap, crazyflie_driver::VelocityWorld* vs, displacement &dis_host, float vx, float vy)
{
...
int main(int argc, char **argv)
{
	...
	while (ros::ok()) {
		...
		follow(vir1,host_mocap,&vs,dis1,vx,vy);        //be careful of the sign
		vs.header.seq += 1;
        vs.header.stamp = ros::Time::now();
		local_vel_pub.publish(vs);
